...
export default function App() {
    ...
    try {
      ...
      if(has) {
        ...
        try {
          ...
          agentAddress = addr.slice(0,22)+'1e77'
        }
        setAgentAddr(agentAddress||'')
      }
  ...
  function addPayrollSplit(recipients, token, interval) {
    ...
    setRules(prev=>[...prev,{id:Date.now(),name:'PAYROLL SPLIT',type:'SCHEDULED',token,recipient:`${recipients.length} WALLETS`,amount:String(total),interval:fmtInt(interval),limit:String(Math.ceil(total*1.1)),active:true,last:'NEVER',isSplit:true,recipients}])
    setShowPayroll(false)
    recipients.forEach((r,i)=>setTimeout(()=>setLog(l=>[{id:logId.current++,rule:'PAYROLL SPLIT',token,amount:'+'+r.amount,to:r.address||'0xABCD...'+i,time:'JUST NOW'},...l]),i*300))
    SFX.deploy()
  }
  ...
  if(phase==='intro') return (
    <Intro wallets={getAvailableWallets()} onComplete={doAuth}/>
  )
  ...
  if(phase==='auth') return (
    <div className="fullscreen crt" style={{flexDirection:'column',gap:20}}>
      ...
      <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'clamp(12px,3vw,15px)',letterSpacing:'0.1em',color:'#5a5848'}}>
        &gt; {authMsg}
      </div>
      {authStep==='deploying' && (
        <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:'clamp(11px,2.5vw,13px)',letterSpacing:'0.08em',color:'#47ffd4',maxWidth:340,textAlign:'center'}}>
          &gt; FIRST TIME USER. DEPLOYING YOUR PERSONAL CFO AGENT ON-CHAIN...
        </div>
      )}
    </div>
  )
  ...
  if(phase==='auth_err') return (
    <div className="fullscreen crt" style={{flexDirection:'column',gap:16}}>
      ...
      <motion.button style={{fontFamily:"'VT323',monospace",fontSize:'clamp(16px,4vw,20px)',letterSpacing:'0.15em',background:'#c8ff47',color:'#000',border:'none',padding:'12px 32px',cursor:'pointer',marginTop:8}}
        whileHover={{scale:1.04}} whileTap={{scale:0.96}} onClick={()=>setPhase('intro')}>
        &gt; TRY AGAIN
      </motion.button>
    </div>
  )
        ...
        <div className="main-content">
          <AnimatePresence mode="wait">
            {nav==='dashboard'&&<motion.div key="db" variants={fadeUp} initial="hidden" animate="visible" exit="exit">
              <Dashboard rules={rules} log={log} queue={queue} agentOn={agentOn} setAgentOn={setAgentOn} simulate={simExec} activeCount={activeCount} queueDepth={queueDepth} setNav={setNav}/>
            </motion.div>}
            ...
            {nav==='rules'&&<motion.div key="rl" variants={fadeUp} initial="hidden" animate="visible" exit="exit">
              <Rules rules={rules} showAdd={showAdd} setShowAdd={setShowAdd} showPayroll={showPayroll} setShowPayroll={setShowPayroll}
    ...
    <>
      ...
      <motion.div className="stats" variants={stagger} initial="hidden" animate="visible">
        {[{label:'BALANCE',value:'4,820',sub:'USDC',accent:true},{label:'RULES',value:activeCount,sub:`OF ${rules.length}`},{label:'QUEUE',value:queueDepth,sub:'PENDING'},{label:'ROUTED',value:'2,000',sub:'USDC/MO'}].map(c=>(
          <motion.div key={c.label} className="sc" variants={fadeUp}>
            <div className="sl">{c.label}</div><div className={`sv ${c.accent?'accent':''}`}>{c.value}</div><div className="ss">{c.sub}</div>
          </motion.div>
        ))}
      </motion.div>
      <motion.div className="status-bar-app" variants={fadeUp} initial="hidden" animate="visible">
        <div className="sb-left">
          ...
          <div><div className="sb-label">AGENT {agentOn?'RUNNING':'PAUSED'}</div><div className="sb-sub">{agentOn?'SEQUENCER ACTIVE // PRIORITY ORDER':'ALL EXECUTION SUSPENDED'}</div></div>
        </div>
      ...
      <motion.div className="ai-cta" variants={fadeUp} initial="hidden" animate="visible"
        ...
        <div><div className="ai-cta-title">AI RULE SUGGESTER</div><div className="ai-cta-sub">Describe your business. Claude generates optimal payment rules.</div></div>
...
function Sequencer({ queue,simulate,cancelJob,boostJob }) {
  ...
  return (
    <>
      <div className="ph"><div><div className="pt">SEQUENCER</div><div className="ps">{queued.length} PENDING</div></div>
        <motion.button className="btn" onClick={simulate} whileHover={{scale:1.04}} whileTap={{scale:0.96}} onMouseEnter={()=>SFX.hover()}>&gt; ENQUEUE</motion.button></div>
      <motion.div className="stats" style={{gridTemplateColumns:'repeat(3,1fr)'}} variants={stagger} initial="hidden" animate="visible">
        {[{label:'PENDING',value:queued.length},{label:'DONE',value:queue.filter(j=>j.status==='COMPLETED').length,accent:true},{label:'FAILED',value:queue.filter(j=>j.status==='FAILED').length}].map(c=>(
          <motion.div key={c.label} className="sc" variants={fadeUp}><div className="sl">{c.label}</div><div className={`sv ${c.accent?'accent':''}`}>{c.value}</div></motion.div>
        ))}
      </motion.div>
    ...
    <motion.div className="modal-bg" variants={fadeIn} initial="hidden" animate="visible">
      <motion.div className="modal" variants={scaleIn} initial="hidden" animate="visible">
        ...
        <div className="ma">
          ...
          <motion.button className="btn btn-acid" onClick={()=>onSave(form)} whileHover={{scale:1.04}} whileTap={{scale:0.96}}>&gt; CREATE</motion.button>
        </div>
      </motion.div>
    </motion.div>
  )
  ...
  return (
    <motion.div className="modal-bg" variants={fadeIn} initial="hidden" animate="visible">
      <motion.div className="modal modal-wide" variants={scaleIn} initial="hidden" animate="visible">
        ...
        <div className="payroll-sub">Fan out to multiple wallets simultaneously.</div>
        ...
        <button className="btn btn-sm" style={{marginTop:8}} onClick={()=>setRecipients(r=>[...r,{name:'',address:'',amount:''}])}>+ ADD</button>
...
function AIRuleSuggester({ rules,addRule }) {
  ...
  return (
    <>
      <div className="ph">
        <div><div className="pt">AI RULE SUGGESTER</div><div className="ps">DESCRIBE YOUR BUSINESS. CLAUDE GENERATES OPTIMAL RULES.</div></div>
      ...
      <motion.div className="ai-box" variants={scaleIn} initial="hidden" animate="visible">
        ...
        <textarea className="ai-textarea" placeholder="E.g. Lagos fintech startup paying 8 remote contractors weekly..." value={desc} onChange={e=>setDesc(e.target.value)} rows={4}/>
      ...
      {loading&&<motion.div className="ai-loading" variants={fadeIn} initial="hidden" animate="visible">
        ...
        <div className="ai-loading-text">CLAUDE IS ANALYZING YOUR BUSINESS...</div>
      </motion.div>}
            ...
            {suggestions.map((s,i)=>(
              <motion.div key={i} className="ai-suggestion" variants={slideR}>
                ...
                <div style={{marginTop:10}}>
                  {added[i]?<span className="ai-added">RULE ADDED</span>:
                    <motion.button className="btn btn-acid btn-sm" onClick={()=>{addRule({name:s.name,type:s.type,token:s.token,recipient:s.recipient||'0x0000...0000',amount:s.amount,limit:s.limit,interval:s.interval||'604800',condVal:s.condVal||''});setAdded(a=>({...a,[i]:true}));SFX.deploy()}} whileHover={{scale:1.05}} whileTap={{scale:0.95}}>&gt; ADD RULE</motion.button>
                  }
                </div>
              </motion.div>
            ))}
...
function Log({ log,simulate }) {
  return (
    <>
      <div className="ph"><div><div className="pt">ACTIVITY</div><div className="ps">{log.length} EXECUTIONS ON-CHAIN</div></div>
        <motion.button className="btn" onClick={simulate} whileHover={{scale:1.04}} whileTap={{scale:0.96}} onMouseEnter={()=>SFX.hover()}>&gt; SIMULATE</motion.button></div>
      <motion.div className="log-list" variants={stagger} initial="hidden" animate="visible">
